...
class AlbatrosDigitizer:
    ...
    def initialise(self, fpg_file, ref_clock, fftshift, acclen, bits, spec_per_packet, bytes_per_spectrum, dest_ip, dest_port, dest_mac, prog_tries=3, adc_tries=3, cooldowntime=10):
        ...
        while i < prog_tries:
            try:
                ...
                self.logger.info("Shutting down SNAP Board for {} second(s).".format(cooldowntime))
                snap_reset(cooldowntime)
                self.logger.info("SNAP reset complete.")
        ...
        print("\nDEBUG (steve): fpga.registers\n{}\n\n".format(self.fpga.registers)) # steve
        print("\nDEBUG (steve): fpga.devices\n{}\n\n".format(self.fpga.devices.keys())) # steve
        print("\nDEBUG (steve): fpga.memory_devices.keys()\n{}\n\n".format(self.fpga.memory_devices.keys())) # steve
        print("\nDEBUG (steve): fpga.adc_devices\n{}\n\n".format(self.fpga.adc_devices)) # steve
        print("\nDEBUG (steve): fpga.other_devices\n{}\n\n".format(self.fpga.other_devices.keys())) # steve
        print("\nDEBUG (steve): fpga.gbes\n{}\n\n".format(self.fpga.gbes)) # steve
        print("\nDEBUG (steve): fpga.sbrams\n{}\n\n".format(self.fpga.sbrams)) # steve
        print("\nDEBUG (steve): fpga.qdrs\n{}\n\n".format(self.fpga.qdrs)) # steve
        print("\nDEBUG (steve): fpga.hmcs\n{}\n\n".format(self.fpga.hmcs)) # steve
        print("\nDEBUG (steve): fpga.snapshots\n{}\n\n".format(self.fpga.snapshots)) # steve
        print("\nDEBUG (steve): fpga.system_info\n{}\n\n".format(self.fpga.system_info)) # steve
        print("\nDEBUG (steve): fpga.rcs_info\n{}\n\n".format(self.fpga.rcs_info)) # steve
        ...
        #%%------Steve edit BEGIN---------------------------------------
        #self.logger.warning("Bypassing setting output bits, assumes #bits=4")
        #assert bits == 4, "firmware has been modified, #bits must equal 4"
        self.logger.info("Setting ouput bits")
        # [Steve] new mux logic
        if bits==1:
            self.fpga.registers.output_mux_sel.write_int(0)
        elif bits==4:
            self.fpga.registers.output_mux_sel.write_int(1)
        else:
            raise Exception("Logic error, bits can only be set to 1 or 4.")
        ## Old mux logic
        ...
        mac=int(dest_mac, 16)
        print("DEBUG: dest_mac={}\n".format(dest_mac)) # Steve
        #%%------steve
        request_args = ("gbe_output_one_gbe", str(0x3000+8*5), struct.pack(">Q",mac))
        #reply,informs = self.fpga.transport.katcprequest(
        #        name="write",
        #        request_timeout=5.0,
        #        require_ok=True,
        ...
        #print("DEBUG: informs",informs)
        devs = self.fpga.transport.listdev()
        print("DEBUG: devs", devs)
        #%%------steve
        for i in range(256):
            # What does this do? [Steve]
        ...
        if (gbe_overflow):
            self.logger.warning("GbE transmit overflowing")
        else:
            self.logger.info("No Gbe overflows detected")
        self.logger.info("Initialisation complete")
        return True

    # [Steve]: I'm worried about what this method does. I don't
    # understand the logic it's supposed to implement. 
    def set_channel_order(self, channels, bits):
        ...
        if (bits==1):
            ...
            raise Exception("Number of bits invalid "+str(bits))
        self.fpga.write(channel_map, channels.astype(">H").tostring(), offset=0)
        return True

    def set_channel_coeffs(self, coeffs, bits):
        if (bits==1):
            self.logger.info("In one bit mode. No need to write coeffs") # [Steve] What are coeffs? Correspond to power in each channel?
            return True
        coeffs_bram_name=""
        # Commented out by Steve, not going to ever use this
        #if (bits==2):
