...
#        logger.info("Unable to read time from trimble. Using RPi system clock which is unreliable")
    ...
    logger.info("# (15) SNAP cooldown time: {}".format(snap_cooldowntime))
    logger.info("########################################################################################")
    ...
    try:
        ...
        # [Steve edit]==Begin=================== 
        logger.info("Setting channel coeffs")
        albatros_snap.set_channel_coeffs(coeffs, bits) 
        try:
            logger.info("Setting channel order")
            albatros_snap.set_channel_order(chans, bits) 
        except:
            print("WARNING: Bypassing exception raised on setting order")
        # [Steve edit]==End===================== 
    finally:
